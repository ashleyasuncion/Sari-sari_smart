...
;(function() {
  ...
  function cacheDom() {
    var ids = [
      ...
      'closingExpenses', 'closingEarnings', 'closingTotalToday',
  ...
  var strings = {
    en: {
      ...
      closingEarnings: "Today's earnings",
      closingTotal: 'Your profit today:',
      ...
      closingTutorial3: 'Enter the total cash you received today (kinikita ngayon).',
      closingTutorial4: 'Your profit is calculated automatically as cost of goods minus earnings.',
      ...
      navClose: 'Close',
      // Day mode
      dayEarningsLabel: "Today's Earnings",
      ...
      closingSectionEarnings: 'Earnings',
    ...
    fil: {
      ...
      closingEarnings: 'Kinikita ngayon',
      closingTotal: 'Kita mo ngayon:',
      ...
      closingTutorial3: 'Ilagay ang kabuuang cash na natanggap mo ngayon (kinikita ngayon).',
      closingTutorial4: 'Ang iyong kita ay awtomatikong kinakwenta bilang gastos minus kita.',
      ...
      closingSectionEarnings: 'Kita Ngayon',
  ...
  function renderClosingScreen() {
    ...
    if (dom.closingEarnings) dom.closingEarnings.value = state.todayEarnings || '';
    updateClosingTotal();
  }

  function updateClosingTotal() {
    if (!dom.closingExpenses || !dom.closingEarnings || !dom.closingTotalToday) return;
    var expenses = parseFloat(dom.closingExpenses.value) || 0;
    var earnings = parseFloat(dom.closingEarnings.value) || 0;
    dom.closingTotalToday.textContent = formatCurrency(earnings - expenses);
  }
  ...
  function completeDay() {
    ...
    var earnings = parseFloat(dom.closingEarnings ? dom.closingEarnings.value : 0) || 0;
    state.todayExpenses = expenses;
    state.todayEarnings = earnings;

    var totalCashEarnings = getTodayEarnings() + earnings;
    var totalItemsSold = getTodayItemsSold();
    var totalUtang = getTodayUtang();
    var profit = totalCashEarnings - expenses;

    state.history.push({
      date: todayStr(),
      earnings: totalCashEarnings,
      expenses: expenses,
      profit: profit,
    ...
    if (dom.summaryDetails) {
      dom.summaryDetails.innerHTML =
        '<div class="summary-detail-row"><span>Earnings</span><span>' + formatCurrency(totalCashEarnings) + '</span></div>' +
        '<div class="summary-detail-row"><span>Expenses</span><span>' + formatCurrency(expenses) + '</span></div>' +
  ...
  function setupEvents() {
    ...
    if (dom.closingEarnings) dom.closingEarnings.addEventListener('input', updateClosingTotal);
